...
public class ReportCustomersOrderVehicles extends Window {
	...
	public ReportCustomersOrderVehicles() {
		this.init();
	}
	...
	private void createRowsFilter(Grid gridFilter2) {
		...
		Label label = new Label(
				"BÁO CÁO CHI TIẾT BIẾN ĐỘNG LƯỢNG KHÁCH ĐẶT XE THEO THỜI GIAN");
		...
		label = new Label("Loại hình đặt xe");
		label.setStyle("font-weight : bold ");
		label.setParent(hbox);
		...
		chosenbox.setWidth("200px");
		List<String> lst = new ArrayList<String>();
		lst.add("1.Mobile");
		lst.add("2.Tổng đài");
		lst.add("3.Web");
		lst.add("4.Loại khác");
		chosenbox.setModel(new ListModelList<>(lst));
		...
		btnSearch.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (timedateto < timedatefrom) {
					...
					model.setFieldType("Loại hình đặt", PivotField.Type.ROW);
		...
		btnExcel.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				String filename = "bao_cao_chi_tiet_bien_dong_luong_khach_dat_xe_theo_thoi_gian";
...
//						: field.getType() == PivotField.Type.ROW ? String
								: field.getType() == PivotField.Type.ROW ? 
										String.valueOf(data) : String.valueOf(data) + "h";
			}
	...
	private List<List<Object>> getModelforPivotTable() {
		...
		Set<String> lsttypeofbooking = chosenbox.getSelectedObjects();
		StringBuilder lsttypeofbookingused = new StringBuilder();
		lsttypeofbookingused.append("0");
		Iterator<String> ite = lsttypeofbooking.iterator();
		while (ite.hasNext()) {
			String s = ite.next();
			lsttypeofbookingused.append("," + s);
		}

		ListObjectDatabase lst = new ListObjectDatabase();
		List<List<Object>> lstValue = lst.getrptCustomersOrderVehiclesTT(
				timedatefrom, timedateto, "" + lsttypeofbookingused);
		return lstValue;
	}
	...
	public void exportToReportViewer(Event event) {
		try {
			if (event.getTarget().equals(btnReport)) {
				...
				mapParams.put("typeofbooking", lsttypeofbookingused.toString());

				String reportFile = "rptDetailCustomerOrderVehicles";
				String reportTitle = "TỔNG ĐÀI ĐIỀU HÀNH - BÁO CÁO CHI TIẾT BIẾN ĐỘNG LƯỢNG KHÁCH ĐẶT XE THEO THỜI GIAN";
				String exportFileName = "bao_cao_chi_tiet_bien_dong_luong_khach_dat_xe_theo_thoi_gian";
				String reportName = "BÁO CÁO CHI TIẾT BIẾN ĐỘNG LƯỢNG KHÁCH ĐẶT XE THEO THỜI GIAN";
