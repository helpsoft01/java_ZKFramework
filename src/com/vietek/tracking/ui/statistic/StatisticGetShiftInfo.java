...
public class StatisticGetShiftInfo extends Window implements
		EventListener<Event> {
	/**
	 * 
	 */
	...
	public StatisticGetShiftInfo() throws Exception {
		this.init();
	}
	...
	private void createData(Hlayout hlayout) throws Exception {
		...
		gridSearchData.setWidth("2320px");
		...
		gridSearchData.setRowRenderer(new RowRenderer<RptGetShiftInfo>() {
			...
			public void render(Row row, RptGetShiftInfo data, int index)
				...
				detail.addEventListener(Events.ON_OPEN, new EventListener<Event>() {
					...
					@Override
					public void onEvent(Event arg0) throws Exception {
						int shiftId = data.getShiftId();
						createGridDetail(detail, shiftId);
						row.appendChild(detail);
					}
				});
				
				row.appendChild(new Label("" + ++index));
				btnPrintTrip = new Button("In cuốc");
				btnPrintTrip
	...
	private void createCboState(Hbox hbox) {
		String[] titles = { "Chưa thu tiền", "Đã thu tiền", "Tất cả" };
		int[] values = { 0, 1, 2 };
	...
	public void createGridDetail(Detail detail, int shiftId) {
		gridDetail = new Grid();
		gridDetail.setParent(detail);
		gridDetail.setWidth("2040px");
		gridDetail.setVflex("1");
		gridDetail.setMold("paging");
		gridDetail.setPageSize(5);
		gridDetail.setClass("gridreportTrip");
		gridDetail.setStyle("overflow : auto");
		...
		auxheader.setLabel("KẾT QUẢ THỐNG KÊ");
		auxheader
				.setStyle("color : black; font-size : 12px; font-weight : bold; margin-left : 50px");
		Columns cols = new Columns();
		...
		gridDetail.setRowRenderer(new RowRenderer<RptTripSearchingOnline>() {
			...
			public void render(Row row, RptTripSearchingOnline data, int index)
				...
				btnHistory.addEventListener(Events.ON_CLICK,
		...
		lstdataDetail = displayRptTripSearchingOnlineDetail(shiftId);
		if (lstdataDetail == null || lstdataDetail.isEmpty()) {
			gridDetail.setEmptyMessage("Không có dữ liệu");
		} else {
			gridDetail.setModel(new ListModelList<RptTripSearchingOnline>(
					lstdataDetail));
		}
